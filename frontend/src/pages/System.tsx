...
export default function SystemPage() {
  ...
  useEffect(() => {
    apiService.getGateways().then((res) => setGateways(res.gateways || [])).catch(console.error);
    apiService.getSystemStats().then((res) => setStats(res)).catch(console.error);
  }, []);

  const systemMetrics = stats ? [
    { label: 'CPU Serveur', value: stats.cpuUsage, max: 100, unit: '%', color: 'bg-green-500' },
    { label: 'Mémoire RAM', value: stats.memoryUsed, max: stats.totalMem, unit: 'MB', color: 'bg-blue-500' },
    { label: 'Uptime', value: Math.round(stats.uptime / 60), max: 1000, unit: 'min', color: 'bg-purple-500' },
  ] : [];
          ...
          <div className="space-y-2">
            {gateways.map((gw) => (
              <div key={gw.id} className="flex items-center gap-3 px-3 py-2 bg-[#0f172a] rounded">
                <span
                  className={`w-2 h-2 rounded-full ${gw.status === 'online' ? 'bg-green-400' : 'bg-red-400'}`}
                />
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span
                      className={`${gw.battery < 70 ? 'text-orange-400' : 'text-green-400'} text-[10px]`}
                    >
                      {gw.battery}%
                    </span>
                  </div>
      ...
      <div className="bg-[#1e293b] rounded border border-gray-700 p-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          {services.map((service) => (
            <div
              key={service.name}
              className="flex items-center gap-3 px-3 py-2.5 bg-[#0f172a] rounded"
            >
