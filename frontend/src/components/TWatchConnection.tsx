...
export default function TWatchConnection({
  onDeviceConnected,
  onDeviceDisconnected,
}: TWatchConnectionProps) {
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      if (result.devices && result.devices.length > 0) {
        const foundDevices: TWatchDevice[] = result.devices.map((d: any) => ({
          id: d.id,
          name: d.name || 'Dispositif Inconnu',
          macAddress: d.id,
          status: 'disconnected',
          batteryLevel: d.battery || 100,
          signalStrength: Math.max(0, 100 + (d.rssi || -70)),
        }));
        setDevices(foundDevices);
        setConnectionStatus('idle');
      } else {
        setErrorMessage('Aucun appareil trouvé');
        setConnectionStatus('error');
      }
    } catch (err) {
      console.error(err);
      setErrorMessage('Erreur lors du scan');
      setConnectionStatus('error');
    } finally {
      setScanning(false);
    }
  ...
  const handleSync = () => {
    if (selectedDevice) {
      ...
      setTimeout(() => {
        setDevices((prev) =>
          prev.map((d) => (d.id === selectedDevice.id ? { ...d, lastSync: new Date() } : d)),
        );
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-gray-300">
                {selectedDevice.lastSync &&
                  `Sync: ${selectedDevice.lastSync.toLocaleTimeString('fr-FR')}`}
              </span>
            </div>
          </div>
        </div>
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-2">
                  <Battery
                    size={12}
                    className={device.batteryLevel < 20 ? 'text-red-400' : 'text-gray-400'}
                  />
