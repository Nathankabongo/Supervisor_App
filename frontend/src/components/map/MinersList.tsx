...
import { Heart, Search, Bell, Wifi, Radio } from 'lucide-react';
  ...
  return (
    <div className="bg-[#1e293b] rounded-xl p-3 border border-gray-700 h-full flex flex-col">
      <div className="flex items-center justify-between mb-2 shrink-0">
        <h3 className="text-xs font-semibold text-white">MINEURS ({miners.length})</h3>
      </div>
      ...
      <div className="space-y-1 flex-1 overflow-y-auto min-h-0">
        {filteredMiners.map((miner) => (
          ...
          >
            {/* Avatar */}
            <div
              className={`w-6 h-6 rounded-full ${getStatusBg(miner.status)} flex items-center justify-center text-white font-bold text-[10px] shrink-0`}
            >
              {miner.name
                .split(' ')
                .map((n) => n[0])
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5">
                ...
                {miner.isInService ? (
                  <span className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse" title="En ligne / Actif" />
                ) : (
                  <span className="w-1.5 h-1.5 bg-gray-600 rounded-full" title="Hors service / Inactif" />
                )}
                {miner.isInService && miner.connectionType === 'wifi' && (
                  <Wifi size={10} className="text-cyan-400" />
                )}
                {miner.isInService && miner.connectionType === 'lora' && (
                  <Radio size={10} className="text-purple-400" />
                )}
              </div>
              <p className="text-[9px] text-gray-400 truncate">
                {miner.isInService ? miner.currentZone : 'Hors ligne'}
              </p>
            </div>
            ...
            <div className="flex items-center gap-0.5 shrink-0">
              ...
              <span className="text-[9px] text-gray-400">
                {Math.round((miner.heartRate / 130) * 100)}%
              </span>
            </div>

            {/* Alert Indicator */}
            {miner.status !== 'safe' && <Bell size={12} className={getStatusColor(miner.status)} />}
          </div>
        ))}
      </div>
