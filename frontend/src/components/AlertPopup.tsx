...
export default function AlertPopup({ alert, onClose, onResolve }: AlertPopupProps) {
  ...
  const getSeverityColor = () => {
    switch (alert.severity) {
      case 'high':
        return 'border-accent-red bg-red-900/20';
      case 'medium':
        return 'border-accent-orange bg-orange-900/20';
      case 'low':
  ...
  return (
    <div
      className={`fixed top-20 right-4 w-96 rounded-lg border-2 p-4 shadow-lg transition-all duration-300 ${getSeverityColor()} ${isVisible ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'}`}
    >
