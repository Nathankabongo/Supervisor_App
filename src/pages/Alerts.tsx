...
export default function Alerts() {
  ...
  const getSeverityColor = (severity: string) => {
    switch (severity) {
      case 'high': return 'text-accent-red bg-red-900/20 border-accent-red';
      case 'medium': return 'text-accent-orange bg-orange-900/20 border-accent-orange';
      case 'low': return 'text-yellow-500 bg-yellow-900/20 border-yellow-500';
      default: return 'text-accent-red bg-red-900/20 border-accent-red';
    }
  };
    ...
    <div className={`bg-card rounded-lg p-4 border ${getSeverityColor(alert.severity)}`}>
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2">
          <AlertTriangle size={20} className={alert.severity === 'high' ? 'text-accent-red' : 'text-accent-orange'} />
