...
import { Gem, TrendingUp, TrendingDown, Clock, Weight, Mountain, BarChart3, Droplets, Flame, Zap, ArrowUpRight, ArrowDownRight, Calendar, Wind, CloudRain, Eye } from 'lucide-react';
...
interface DailySummary {
  ...
  totalCarats: number;
  totalTons: number;
  minerals: { name: string; quantity: number; unit: string }[];
}
...
const defaultExtractions: ExtractionData[] = [
  { id: 'ext-1', mineral: 'Diamant', mineralIcon: '💎', quantity: 120, unit: 'carats', todayQuantity: 120, yesterdayQuantity: 98, target: 150, zone: 'ZONE A', gallery: 'Galerie A1', quality: 'Premium', lastUpdate: new Date().toISOString() },
  { id: 'ext-2', mineral: 'Or', mineralIcon: '🥇', quantity: 2.4, unit: 'kg', todayQuantity: 2.4, yesterdayQuantity: 2.1, target: 3.0, zone: 'ZONE B', gallery: 'Galerie B2', quality: 'Haute', lastUpdate: new Date().toISOString() },
  { id: 'ext-3', mineral: 'Fer', mineralIcon: '⛏️', quantity: 45, unit: 'tonnes', todayQuantity: 45, yesterdayQuantity: 52, target: 60, zone: 'ZONE C', gallery: 'Galerie C1', quality: 'Standard', lastUpdate: new Date().toISOString() },
  { id: 'ext-4', mineral: 'Cuivre', mineralIcon: '🔶', quantity: 8.5, unit: 'tonnes', todayQuantity: 8.5, yesterdayQuantity: 7.8, target: 10, zone: 'ZONE D', gallery: 'Galerie D3', quality: 'Haute', lastUpdate: new Date().toISOString() },
  { id: 'ext-5', mineral: 'Bauxite', mineralIcon: '🪨', quantity: 120, unit: 'tonnes', todayQuantity: 120, yesterdayQuantity: 135, target: 150, zone: 'ZONE E', gallery: 'Galerie E2', quality: 'Standard', lastUpdate: new Date().toISOString() },
  { id: 'ext-6', mineral: 'Phosphate', mineralIcon: '🧪', quantity: 85, unit: 'tonnes', todayQuantity: 85, yesterdayQuantity: 78, target: 100, zone: 'ZONE F', gallery: 'Galerie F1', quality: 'Industrielle', lastUpdate: new Date().toISOString() },
];

const weekHistory = [
  { day: 'Lun', diamant: 95, or: 1.8, fer: 48, cuivre: 7.2 },
  { day: 'Mar', diamant: 110, or: 2.2, fer: 55, cuivre: 8.1 },
  { day: 'Mer', diamant: 88, or: 1.5, fer: 42, cuivre: 6.9 },
  { day: 'Jeu', diamant: 130, or: 2.8, fer: 60, cuivre: 9.2 },
  { day: 'Ven', diamant: 105, or: 2.0, fer: 50, cuivre: 7.5 },
  { day: 'Sam', diamant: 75, or: 1.2, fer: 30, cuivre: 5.0 },
  { day: 'Dim', diamant: 120, or: 2.4, fer: 45, cuivre: 8.5 },
];

export default function Extraction() {
  const [extractions, setExtractions] = useState<ExtractionData[]>(defaultExtractions);
  const [selectedMineral, setSelectedMineral] = useState<string | null>(null);
  const [timeRange, setTimeRange] = useState<'today' | 'week' | 'month'>('today');
  const { weather, isLoading: weatherLoading, description, airQuality, windDirection } = useWeather();
  ...
  const globalProgress = extractions.reduce((sum, e) => sum + (e.todayQuantity / e.target) * 100, 0) / extractions.length;
  ...
  const maxBarValue = Math.max(...weekHistory.map(d => d.diamant));
    ...
    <div className="p-4 space-y-4 bg-[#0f172a] min-h-screen">
              ...
              {(['today', 'week', 'month'] as const).map((range) => (
                <button
                  ...
                  className={`px-3 py-1 rounded text-xs font-medium transition-colors ${
                    timeRange === range ? 'bg-green-500 text-white' : 'text-gray-400 hover:text-white'
                  }`}
                >
      ...
      <div className="grid grid-cols-4 gap-3">
        <div className="bg-[#1e293b] rounded-lg p-4 border border-gray-700">
          ...
          <p className="text-2xl font-bold text-white">{totalCarats} <span className="text-sm text-gray-400">carats</span></p>
        ...
        <div className="bg-[#1e293b] rounded-lg p-4 border border-gray-700">
          ...
          <p className="text-2xl font-bold text-white">{totalKg} <span className="text-sm text-gray-400">kg</span></p>
          <div className="flex items-center gap-1 mt-1">
            <ArrowUpRight size={12} className="text-green-400" />
        ...
        <div className="bg-[#1e293b] rounded-lg p-4 border border-gray-700">
          ...
          <p className="text-2xl font-bold text-white">{totalTons} <span className="text-sm text-gray-400">tonnes</span></p>
          <div className="flex items-center gap-1 mt-1">
        ...
        <div className="bg-[#1e293b] rounded-lg p-4 border border-gray-700">
          ...
          <p className="text-2xl font-bold text-white">{globalProgress.toFixed(0)}<span className="text-sm text-gray-400">%</span></p>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="col-span-2 space-y-3">
            ...
            <div className="p-3 space-y-2">
              {extractions.map((extraction) => {
                ...
                return (
                  <div
                    key={extraction.id}
                    onClick={() => setSelectedMineral(selectedMineral === extraction.id ? null : extraction.id)}
                    ...
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-gray-400">{extraction.zone} • {extraction.gallery}</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-xl font-bold text-white">
                          {extraction.todayQuantity} <span className="text-xs text-gray-400">{extraction.unit}</span>
                        </p>
                        <div className="flex items-center gap-1 justify-end">
                          ...
                          <span className={`text-xs ${trend.isUp ? 'text-green-400' : 'text-red-400'}`}>
                            {trend.isUp ? '+' : '-'}{trend.value}%
                          </span>
                          <span className="text-[10px] text-gray-500">vs hier ({extraction.yesterdayQuantity})</span>
                        </div>
                      </div>
                    </div>
                    ...
                    {selectedMineral === extraction.id && (
                      <div className="mt-3 pt-3 border-t border-gray-700 grid grid-cols-4 gap-3 text-xs">
                        ...
                        <div>
                          ...
                          <p className="text-white font-medium">{extraction.target} {extraction.unit}</p>
                        </div>
                        <div>
                          <p className="text-gray-400">Rendement</p>
                          <p className={`font-medium ${progress >= 80 ? 'text-green-400' : progress >= 50 ? 'text-amber-400' : 'text-red-400'}`}>
                            {progress.toFixed(1)}%
                          </p>
                        </div>
                        <div>
                          ...
                          <p className="text-white font-medium">
                            {new Date(extraction.lastUpdate).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}
                          </p>
                        </div>
                      </div>
                    )}
          ...
          <div className="bg-[#1e293b] rounded-lg border border-gray-700">
            <div className="p-4 border-b border-gray-700">
              <h2 className="text-sm font-semibold text-white">ÉVOLUTION HEBDOMADAIRE - DIAMANTS (CARATS)</h2>
            </div>
            <div className="p-4">
              <div className="flex items-end gap-3 h-40">
                {weekHistory.map((day, i) => {
                  const height = (day.diamant / maxBarValue) * 100;
                  const isToday = i === weekHistory.length - 1;
                  return (
                    <div key={day.day} className="flex-1 flex flex-col items-center gap-1">
                      <span className="text-xs text-white font-medium">{day.diamant}</span>
                      <div
                        className={`w-full rounded-t transition-all ${isToday ? 'bg-blue-500' : 'bg-blue-500/40'}`}
                        style={{ height: `${height}%` }}
                      />
                      <span className={`text-[10px] ${isToday ? 'text-white font-bold' : 'text-gray-400'}`}>
                        {day.day}
                      </span>
                    </div>
                  );
        ...
        <div className="space-y-3">
          ...
          <div className="bg-[#1e293b] rounded-lg border border-gray-700">
            ...
            <div className="p-3 space-y-2">
              {['A', 'B', 'C', 'D', 'E', 'F'].map((zone) => {
                ...
                return (
                  <div key={zone} className="flex items-center justify-between p-2 bg-[#0b1a2a] rounded border border-gray-700">
            ...
            <div className="p-4 border-b border-gray-700 flex items-center justify-between">
              ...
              {weather && (
                <span className="text-[10px] text-gray-500">
                  Météo temps réel • {new Date(weather.updatedAt).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}
                </span>
              )}
            </div>
                ...
                <>
                  ...
                  <div className="flex items-center justify-between p-2 bg-[#0b1a2a] rounded border border-gray-700">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-xs font-medium ${description?.color}`}>{description?.label}</span>
                    </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <span className="text-sm text-white font-medium">{weather.temperature}°C</span>
                      <span className="text-[10px] text-gray-500 ml-1">Ressenti {weather.apparentTemperature}°C</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm text-white font-medium">{weather.precipitation} mm</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm text-white font-medium">{weather.pressure.toFixed(0)} hPa</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-sm font-medium ${airQuality?.color}`}>{airQuality?.label}</span>
                  </div>
                </>
              ) : (
          ...
          <div className="bg-[#1e293b] rounded-lg border border-gray-700">
            ...
            <div className="p-3 space-y-2">
              {[
                { time: '14:32', text: 'Diamant 15 carats trouvé - Zone A', type: 'success' },
                { time: '13:15', text: 'Filon de cuivre détecté - Zone D', type: 'info' },
                { time: '11:45', text: 'Maintenance foreuse - Zone C', type: 'warning' },
                { time: '09:30', text: 'Démarrage extraction - Zone E', type: 'info' },
                { time: '08:00', text: 'Briefing sécurité équipe', type: 'default' },
              ].map((event, i) => (
                <div key={i} className="flex items-start gap-2 text-xs">
                  <span className="text-gray-500 min-w-[35px]">{event.time}</span>
                  <span className={`${
