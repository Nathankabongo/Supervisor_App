...
const mockUsers: UserItem[] = [
  { id: '1', name: 'John Supervisor', email: 'john@nickelfox.com', role: 'Superviseur', status: 'active', lastLogin: '03/05/2026 03:00' },
  { id: '2', name: 'Marie Dupont', email: 'marie@nickelfox.com', role: 'Opérateur', status: 'active', lastLogin: '03/05/2026 02:45' },
  { id: '3', name: 'Pierre Martin', email: 'pierre@nickelfox.com', role: 'Technicien', status: 'active', lastLogin: '02/05/2026 18:30' },
  { id: '4', name: 'Sophie Bernard', email: 'sophie@nickelfox.com', role: 'Ingénieur', status: 'inactive', lastLogin: '28/04/2026 09:15' },
  { id: '5', name: 'Luc Petit', email: 'luc@nickelfox.com', role: 'Administrateur', status: 'active', lastLogin: '03/05/2026 01:00' },
  { id: '6', name: 'Isabelle Moreau', email: 'isabelle@nickelfox.com', role: 'Opérateur', status: 'active', lastLogin: '03/05/2026 02:30' },
];
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">{mockUsers.length} utilisateurs enregistrés</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-[#1e293b] rounded border border-gray-700 p-3 flex items-center gap-3">
          ...
          <div>
            <p className="text-white text-lg font-bold">{mockUsers.filter(u => u.status === 'active').length}</p>
        ...
        <div className="bg-[#1e293b] rounded border border-gray-700 p-3 flex items-center gap-3">
          ...
          <div>
            <p className="text-white text-lg font-bold">{mockUsers.filter(u => u.status === 'inactive').length}</p>
        ...
        <table className="w-full">
          ...
          <tbody>
            {filteredUsers.map(user => (
              <tr key={user.id} className="border-b border-gray-700/50 hover:bg-[#334155] transition-colors">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    <div className={`w-8 h-8 ${user.status === 'active' ? 'bg-green-500' : 'bg-gray-500'} rounded-full flex items-center justify-center text-white text-xs font-bold`}>
                ...
                <td className="px-4 py-3">
                  <span className={`flex items-center gap-1 text-xs ${user.status === 'active' ? 'text-green-400' : 'text-gray-400'}`}>
                    <span className={`w-2 h-2 rounded-full ${user.status === 'active' ? 'bg-green-400' : 'bg-gray-500'}`} />
