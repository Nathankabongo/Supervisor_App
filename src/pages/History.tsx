...
export default function HistoryPage() {
  ...
  // Generate history events from miners and alerts
  const generateEvents = (): HistoryEvent[] => {
    const events: HistoryEvent[] = [];
    
    miners.forEach(miner => {
      // Zone changes from trajectory
      if (miner.trajectory.length > 1) {
        events.push({
          id: `zone-${miner.id}`,
          type: 'zone_change',
          minerName: miner.name,
          minerId: miner.id,
          description: `Déplacé vers ${miner.currentZone}`,
          timestamp: miner.position.timestamp,
          zone: miner.currentZone,
        });
      }
      ...
      events.push({
        ...
        minerName: miner.name,
        minerId: miner.id,
        description: `Entrée dans la mine - ${miner.currentZone}`,
        timestamp: Date.now() - Math.floor(Math.random() * 28800000),
        zone: miner.currentZone,
      });
    });
    ...
    alerts.forEach(alert => {
      events.push({
        ...
        description: `Alerte: ${alert.type} - ${alert.severity}`,
  ...
  const filteredEvents = events.filter(e => {
    const matchesSearch = e.minerName.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      case 'zone_change': return <MapPin size={16} className="text-blue-400" />;
      case 'alert': return <AlertTriangle size={16} className="text-red-400" />;
      case 'status_change': return <User size={16} className="text-orange-400" />;
      case 'entry': return <History size={16} className="text-green-400" />;
      case 'exit': return <Clock size={16} className="text-gray-400" />;
  ...
  const getTypeBg = (type: string) => {
    switch (type) {
      case 'zone_change': return 'bg-blue-500/20 border-blue-500/30';
      case 'alert': return 'bg-red-500/20 border-red-500/30';
      case 'status_change': return 'bg-orange-500/20 border-orange-500/30';
      ...
      <div className="bg-[#1e293b] rounded border border-gray-700 p-4">
        ...
        <div className="grid grid-cols-4 gap-3">
          <div className="text-center">
            <p className="text-lg font-bold text-green-400">{events.filter(e => e.type === 'entry').length}</p>
            <p className="text-xs text-gray-400">Entrées</p>
          </div>
          <div className="text-center">
            <p className="text-lg font-bold text-blue-400">{events.filter(e => e.type === 'zone_change').length}</p>
            <p className="text-xs text-gray-400">Déplacements</p>
          </div>
          <div className="text-center">
            <p className="text-lg font-bold text-red-400">{events.filter(e => e.type === 'alert').length}</p>
            <p className="text-xs text-gray-400">Alertes</p>
          </div>
          <div className="text-center">
            <p className="text-lg font-bold text-gray-400">{events.filter(e => e.type === 'exit').length}</p>
