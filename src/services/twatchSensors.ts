...
class TWatchSensorsService {
  ...
  private generateGPSData(): GPSData {
    ...
    if (lastGPS) {
      // Petit mouvement depuis la dernière position
      const latChange = (Math.random() - 0.5) * 0.0001;
      const lonChange = (Math.random() - 0.5) * 0.0001;
      
      return {
        latitude: lastGPS.latitude + latChange,
        longitude: lastGPS.longitude + lonChange,
        altitude: lastGPS.altitude + (Math.random() - 0.5) * 2,
  ...
  detectFall(): boolean {
    ...
    for (const data of recent) {
      ...
      if (magnitude > 20) { // Seuil de chute
        return true;
      }
    }
