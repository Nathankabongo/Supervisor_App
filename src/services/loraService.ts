...
class LoRaService {
  ...
  private notifyListeners(message: LoRaMessage): void {
    this.listeners.forEach(callback => callback(message));
  }

  // Envoyer des données de localisation
  async sendLocation(deviceId: string, position: { x: number; y: number; z?: number }): Promise<boolean> {
  ...
  async sendHealthData(deviceId: string, healthData: {
    heartRate: number;
  ...
  // Envoyer une alerte
  async sendAlert(deviceId: string, alert: {
  ...
  async sendStatus(deviceId: string, status: {
    batteryLevel: number;
