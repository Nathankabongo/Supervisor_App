...
import dataRoutes, { initTraceData, addTraceEventFromSimulator } from './routes/data.js';
...
import { LoRaSimulator } from './services/loraSimulator.js';
...
// Simulateur LoRa (activé si pas de montre physique connectée)
let loraSimulator = null;
const USE_SIMULATOR = process.env.USE_SIMULATOR !== 'false'; // Activé par défaut

if (USE_SIMULATOR) {
  loraSimulator = new LoRaSimulator((data) => {
    realTimeDataService.processLoRaData(data);
  });
}

// Injecter les références dans les routes LoRa
setLoraReferences(loraSimulator, serialPortManager);

// Initialiser les données de traçabilité
initTraceData();

// Connecter le simulateur LoRa au système de traçabilité
if (loraSimulator) {
  loraSimulator.onTraceEvent = addTraceEventFromSimulator;
}
...
const gracefulShutdown = (signal) => {
  ...
  if (loraSimulator) loraSimulator.stop();
...
httpServer.listen(PORT, () => {
  ...
  // Démarrer la connexion série LoRa (uniquement si pas en mode simulateur)
  if (!loraSimulator) {
    serialPortManager.start().catch(err => {
      console.warn('Port série non disponible:', err.message);
    });
  } else {
    console.log('Port série ignoré (mode simulateur actif)');
  }
  
  // Démarrer le simulateur si activé
  if (loraSimulator) {
    loraSimulator.start();
    console.log('Simulateur LoRa actif (données de montre simulées)');
  } else {
